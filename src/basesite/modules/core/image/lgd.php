 ...
 * @copyright © 2018
 */
class Core_Image_Lgd extends Core_Image
{
	/**
	 ...
	 * @return bool
	 */
	static public function resizeImage($sourceFile, $maxWidth, $maxHeight, $targetFile, $quality = NULL, $preserveAspectRatio = TRUE, $createPrev = TRUE)
	{
		...
		if ($createPrev and (defined('CREATE_IMP_PREV') and CREATE_IMP_PREV))
		{
			$destX = $destX_step2;
			$destY = $destY_step2;
			...
			if ($destX > $destY)
			{
				$destY = $destY *70 / $destX;
				$destX = 70;
			}
			else
			{
				$destX = $destX *70 / $destY;
				$destY = 70;
			}
			$pos = strrpos($targetFile, '/')+1;
			$newtargetFile = substr($targetFile, 0, $pos).'p_'.substr($targetFile, $pos);

			Core_Message::show("Превью '"."p_".substr($targetFile, $pos)."'' создано.");
 			Core_Image::instance()->resizeImage($targetFile, $destX, $destY, $newtargetFile, 10, TRUE, FALSE);
		}
		return TRUE;
	}
